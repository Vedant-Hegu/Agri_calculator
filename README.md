...
Key Features
•	Seed Estimator:
o	Calculates the total number of seeds based on the aggregate weight of a batch. 
o	Utilizes predefined reference weights (measured in grams per 50 seeds) for specific crops: Pavta (Lima Bean), Matar (Green Pea), and Mung (Green Gram). 
•	Germination Planner:
o	Predicts expected plant viability based on pre-configured historical germination rates, such as an 84% success rate for Pavta. 
o	Calculates the required surplus of seeds a user must sow to achieve a specific target yield. 
•	Growth Comparison Analysis:
o	Compares crop germination performance across differing environmental conditions, such as "Soil Controlled" or "Humidity and Moisture Control," against a "No Control" baseline. 
o	Renders an interactive line graph to visualize these comparative growth metrics. 
o	Displays crop-specific analytical insights, detailing optimum parameter ranges (e.g., temperatures of 21°C to 23°C) required for maximum yield. 
•	Environmental Monitoring Log:
o	Visualizes time-stamped, historical sensor readings for Temperature, Humidity, and Soil Moisture. 
o	Computes overall statistical averages for the active monitoring dataset. 
o	Incorporates an automated alert system that flags abnormal readings, warning the user if recorded temperatures exceed 30°C or if soil moisture levels drop below 65. 
